...
public class ApplicationDbContext : DbContext
{
    ...
    public UnitReply QueryTiuIdUnitData(string tiuId, string site, uint dataNumberOfDay)
    {
        if (site == "PG12" || site == "IDC" || site == "JF")
            site = "LABS";
        // Construct the table name using the site parameter
        ...
        var headerquery = $@"
            ...
            WHERE 
                [uh].[tuiId] = ? 
                AND CAST([ud].[added_date] AS DATE) >= DATEADD(day, -{dataNumberOfDay}, CAST(GETDATE() AS DATE))
                AND CAST([ud].[added_date] AS DATE) <= CAST(GETDATE() AS DATE)
        ";
