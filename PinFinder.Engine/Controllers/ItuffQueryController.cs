...
public interface IItuffQueryController
{
    ...
    Task<UnitReply> GetTiuIdUnitData(string tiuId, string site, uint dataNumberOfDay);
}
...
public class ItuffQueryController : IItuffQueryController
{
    ...
    public async Task<UnitReply> GetTiuIdUnitData(string tiuId, string site, uint dataNumberOfDay)
    {
        return await _mediator.Send(new TiuIdCall() { TiuId = tiuId,  Site = site, DataNumberOfDay = dataNumberOfDay });
    }
}
